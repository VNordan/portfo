class Organization < ActiveRecord::Base
  ...
  alias_attribute 'post' , 'DolzhnostKL'
  alias_attribute 'second_name' , 'FamilyKL'
  alias_attribute 'first_name' , 'NameKL'
  alias_attribute 'last_name' , 'OtchestvoKL'
  alias_attribute 'phone' , 'PhoneKL'
  ...
  def self.list
    orgs = [];
    Organization.includes(:objs).each do |o|
      next if o.objs.count ==0
      orgs << {
          id: o.id,
          name: o.name,
          object_count: (o.objs.count||0).to_s,
          contest_win_count: (o.object_tenders.count||0).to_s,
          ceo: "#{o.second_name} #{o.first_name} #{o.last_name}",
          address: 'Юр Адрес'
      }
    end
    orgs
  ...
  def self.organizations_parts_in_sum
    ...
    Organization.all.each do |o|
      o.object_tenders.where(status: 'проведен').
          group("YEAR(DataFinish)").sum('TenderPriceEnd').each do |year, summT|
        orgs << {
            id: Date.parse("#{year}-01-01").to_time.to_i+o.id,
            contractor_id: o.id,
  ...
  def self.organizations_contracts
    ...
    ConstructionObject.only_active_with_sql.each do |o|
      next if o.obj.organization.nil?
      contracts << {
          contractor_id: o.obj.organization.try(:id),
          object_id: o.id,
          status: o.obj.status_name,
          network: "#{o.obj.SMR_external_network||'-'}/#{o.obj.SMR_external_network_delay||0}",
          erection: "#{o.obj.SMR_constructive||'-'}/#{o.obj.SMR_constructive_delay||0}",
          finishing: "#{o.obj.SMR_internal||'-'}/#{o.obj.SMR_internal_delay||0}"
      }
    end
    contracts
  end
