class ObjectPhoto < ActiveRecord::Base
  ...
  belongs_to :obj, foreign_key: "ObjId"
  ...
  def self.objects_photos
    ...
    ConstructionObject.includes(:obj).each do |object|
      ...
      d = ObjectPhoto.select('MAX(CAST(PhotoDate AS DATE)) as max_date').group('ObjID').where("ObjID = ?", object.obj.id).take
  ...
  def self.photos_by_object_and_date obj_id, date
    values = []
    ObjectPhoto.includes(obj: [:construction_object]).where('PhotoDate >= ?', date).where(object_id: obj_id).each do |p|
      next if p.obj.nil?
